...
* 🏗️ **URDF zu Physik (v0, teilweise):** liest echte `<joint>`-Elemente (Typ/Ursprung/Achse/Grenze) und durchläuft die echten `parent`/`child`-Linknamen jedes Gelenks vom eigenen echten Wurzel-Link des Dokuments aus in eine Kette, in echter Wurzel-zu-Blatt-Reihenfolge, unabhängig von der Reihenfolge der `<joint>`-Elemente in der Datei. *Noch nicht real:* die Erzeugung von Kollisionsnetzen - das bleibt die "physikalische" Hälfte dieses Features.
...
```mermaid
flowchart LR
    URDF["Visuelles URDF - real v0 (parent/child-Baumdurchlauf, nur serielle Ketten)"] --> MESH["Kollisionsnetz-Vereinfachung - geplant"]
...
* **Warum v0 eine echt verzweigte URDF ablehnt, statt sie abzuflachen.** Gefunden in einem ökosystemweiten Software-Verbesserungs-Audit: v0 parste früher `<joint>`-Elemente in roher Dokumentreihenfolge und behandelte diese Reihenfolge als die Kette - das ist falsch für jede spec-konforme URDF, die ihre Gelenke außerhalb der Baumreihenfolge deklariert. Behoben: die echten `parent`/`child`-Linknamen jedes Gelenks werden jetzt vom echten Wurzel-Link des Dokuments in echter Wurzel-zu-Blatt-Reihenfolge durchlaufen. `Chain` selbst bleibt ein flacher `Vec<Joint>`, sodass ein echter Zweig (ein Link mit mehr als einem Kind-Gelenk) keine einzelne Reihenfolge hat, in die er abgeflacht werden könnte - er wird mit einem echten Fehler abgelehnt, statt still einen Zweig zu behalten und den anderen zu verwerfen. Ehrlich für den eigenen Katalog von `HYDRA-UMC-EDITOR-URDF`, der heute vollständig aus einzelnen seriellen Armen besteht (siehe `urdf.rs`).
...
│   ├── urdf.rs           # Echter, partieller URDF-Reader (parent/child-Baumdurchlauf, nur serielle Ketten)
...
```bash
# Linux / macOS
./build.sh   # Kilometerzähler-Versions-Bump, `cargo test` (49 Tests), dann `cargo build --release`
