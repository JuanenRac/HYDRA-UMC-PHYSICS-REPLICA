...
**Verifica di onestà - cosa funziona davvero oggi:** `fk --urdf PERCORSO --joints "j1=0.5,..."` calcola vere posizioni nel mondo per ogni giunto concatenando le trasformazioni dei giunti del URDF, indipendentemente dal fatto che una posizione sia nel proprio limite dichiarato; `fk-checked` esegue lo stesso calcolo ma verifica prima ogni posizione contro la vera verifica di `validate-limits`, rifiutandosi di calcolare (o riportare) qualsiasi posa se qualcosa è fuori intervallo; `validate-limits --urdf PERCORSO --joints "..."` segnala veri giunti fuori intervallo per conto proprio. Tutti e tre sono cinematica pura - nessuna dinamica di corpo rigido, nessuna forza di contatto, nessun solutore MuJoCo/PhysX ancora collegato, e il lettore URDF percorre un vero albero di link `parent`/`child` in un'unica catena seriale, rifiutando (senza appiattirlo) un albero realmente ramificato (vedi la documentazione propria del modulo `urdf.rs` per il perché). Vedi [`CHANGELOG.md`](CHANGELOG.md) per ciò che è stato consegnato esattamente, e la Roadmap sotto per ciò che resta da fare.
...
```mermaid
flowchart LR
    URDF["URDF visuale - reale v0 (percorso albero parent/child, solo catene seriali)"] --> MESH["Semplificazione mesh di collisione - previsto"]
...
* **Perché v0 rifiuta un vero URDF ramificato invece di appiattirlo.** Trovato in un audit di miglioramento software a livello di ecosistema: v0 analizzava prima gli elementi `<joint>` nell'ordine grezzo del documento e trattava quell'ordine come la catena, il che è sbagliato per qualsiasi URDF conforme alle specifiche che dichiara i propri giunti fuori dall'ordine dell'albero. Corretto: i veri nomi di link `parent`/`child` di ogni giunto vengono ora percorsi dal vero link radice del documento in un vero ordine radice-foglia. `Chain` stesso resta un `Vec<Joint>` piatto, quindi un vero ramo (un link con più di un giunto figlio) non ha un unico ordine in cui appiattirlo - viene rifiutato con un vero errore invece di mantenere silenziosamente un ramo e scartare l'altro. Onesto per il proprio catalogo di `HYDRA-UMC-EDITOR-URDF`, oggi interamente composto da bracci seriali singoli (vedi `urdf.rs`).
...
│   ├── urdf.rs           # Lettore URDF reale e parziale (percorso albero parent/child, solo catene seriali)
...
```bash
# Linux / macOS
./build.sh   # bump di version contachilometri, `cargo test` (49 test), poi `cargo build --release`
