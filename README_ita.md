...
**Verifica di onestà - cosa funziona davvero oggi:** `fk --urdf PERCORSO --joints "j1=0.5,..."` calcola vere posizioni nel mondo per ogni giunto concatenando le trasformazioni dei giunti del URDF; `validate-limits --urdf PERCORSO --joints "..."` segnala veri giunti fuori intervallo. Entrambi sono cinematica pura - nessuna dinamica di corpo rigido, nessuna forza di contatto, nessun solutore MuJoCo/PhysX ancora collegato, e il lettore URDF supporta solo una singola catena seriale (vedi la documentazione propria del modulo `urdf.rs` per il perché). Vedi [`CHANGELOG.md`](CHANGELOG.md) per ciò che è stato consegnato esattamente, e la Roadmap sotto per ciò che resta da fare.
...
│   ├── kinematics.rs        # forward_kinematics() reale
│   ├── limits.rs              # validate_limits() reale
│   └── main.rs                  # Entry point + sottocomandi reali `fk`/`validate-limits`
...
```bash
# Linux / macOS
./build.sh   # bump di version contachilometri, `cargo test` (24 test), poi `cargo build --release`
...
`fk` esce con `0` in caso di successo, `2` se `--urdf`/`--joints` non è valido. `validate-limits` esce con `0` (nessuna violazione), `1` (violazioni trovate), o `2` (input non valido).
