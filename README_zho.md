...
```mermaid
flowchart LR
    URDF["Visual URDF - 真实 v0（parent/child树遍历，仅支持串联链）"] --> MESH["Collision Mesh Simplification - 计划中"]
...
* **为何 v0 拒绝真正分叉的 URDF，而不是把它拍平。** 在一次面向整个生态系统的软件改进审计中发现——v0 之前按文档的原始顺序解析 `<joint>` 元素，并把这个顺序当作链条本身，这对任何按规范声明关节顺序不按树形排列的 URDF 都是错误的。已修复——现在会从文档自身真实的根连杆开始，沿着每个关节真实的 `parent`/`child` 连杆名称，以真正的从根到叶的顺序遍历。`Chain` 本身仍然是一个扁平的 `Vec<Joint>`，因此真正的分叉（一个连杆有多个子关节）没有单一顺序可以拍平——遇到这种情况会用真实错误拒绝，而不是悄悄保留一个分支、丢弃另一个。这对 `HYDRA-UMC-EDITOR-URDF` 自身的目录是诚实的，因为它今天完全由单一串联臂组成（详见 `urdf.rs`）。
...
│   ├── urdf.rs           # 真实的、部分支持的 URDF 读取器（parent/child树遍历，仅支持串联链）
...
```bash
# Linux / macOS
./build.sh   # 里程表式版本递增、`cargo test`（49 个测试），然后执行 `cargo build --release`
