...
```mermaid
flowchart LR
    URDF["URDF Visual - real v0 (recorrido de árbol parent/child, solo cadenas seriales)"] --> MESH["Simplificación de Malla de Colisión - planeado"]
...
* **Por qué v0 rechaza un URDF con ramificación real en vez de aplanarlo.** Encontrado en una auditoría de mejoras de software a nivel de todo el ecosistema: v0 antes analizaba los elementos `<joint>` en el orden bruto del documento y trataba ese orden como la cadena, lo cual es incorrecto para cualquier URDF válido según la especificación que declare sus articulaciones fuera del orden del árbol. Arreglado: los nombres de enlace `parent`/`child` reales de cada articulación ahora se recorren desde el enlace raíz real del documento en un orden real de raíz a hoja. `Chain` en sí sigue siendo un `Vec<Joint>` plano, así que una ramificación real (un enlace con más de una articulación hija) no tiene un único orden en el que aplanarla - se rechaza con un error real en vez de quedarse silenciosamente con una rama y descartar la otra. Honesto para el propio catálogo de `HYDRA-UMC-EDITOR-URDF`, que hoy son enteramente brazos seriales únicos (ver `urdf.rs`).
...
│   ├── urdf.rs           # Lector real y parcial de URDF (recorrido de árbol parent/child, solo cadenas seriales)
...
```bash
# Linux / macOS
./build.sh   # bump de version cuentakilometros, `cargo test` (49 tests), luego `cargo build --release`
