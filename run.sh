...
# *****************************************************************************
# Project   : HYDRA-UMC-PHYSICS-REPLICA
# Script    : run.sh
...
printf '\n*******************************************************************************\n'
printf '%s\n' "* HYDRA-UMC-PHYSICS-REPLICA - run.sh"
printf '%s\n' "* Mode      : RUN WORKFLOW"
printf '%s\n' "* Author    : JuanenRac (Electro Hobby 3D)"
printf '%s\n' "* Email     : electrohobby3d@gmail.com"
printf '%s\n' "* Copyright : (C) 2026 JuanenRac"
...
# Forwards all arguments (e.g. "./run.sh fk --urdf arm.urdf --joints j1=0.5").
set -euo pipefail
cd "$(dirname "$0")"

BIN="target/release/hydra-umc-physics-replica"
