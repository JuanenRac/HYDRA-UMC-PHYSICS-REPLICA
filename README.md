...
* 🏗️ **URDF to Physics (v0, partial):** reads real `<joint>` elements (type/origin/axis/limit) and walks each joint's real `parent`/`child` link names from the document's own real root link into a chain, in genuine root-to-leaf order regardless of `<joint>` element order in the file. *Not yet real:* collision mesh generation - that's still the "physics" half of this feature.
...
```mermaid
flowchart LR
    URDF["Visual URDF - real v0 (parent/child tree walk, serial chains only)"] --> MESH["Collision Mesh Simplification - planned"]
...
* **Why v0 refuses a real branching URDF instead of flattening it.** Found in an ecosystem-wide software-improvements audit: v0 used to parse `<joint>` elements in raw document order and treat that order as the chain, which is wrong for any spec-valid URDF that declares its joints out of tree order. Fixed: every joint's real `parent`/`child` link names are now walked from the document's own real root link into genuine root-to-leaf order. `Chain` itself is still a flat `Vec<Joint>`, so a real branch (one link with more than one child joint) has no single order to flatten it into - refused with a real error rather than silently keeping one branch and dropping the other. Honest for `HYDRA-UMC-EDITOR-URDF`'s own catalog, which is entirely single serial arms today (see `urdf.rs`).
...
│   ├── urdf.rs           # Real, partial URDF reader (parent/child tree walk, serial chains only)
...
```bash
# Linux / macOS
./build.sh   # odometer version bump, `cargo test` (49 tests), then `cargo build --release`
