...
ce projet ne comporte donc pas de dossiers `hardware/`, `firmware/` ni
`os/` (voir la règle d'élagage dans
...
│   ├── kinematics.rs        # forward_kinematics() réel
│   ├── limits.rs              # validate_limits() réel
│   └── main.rs                  # Point d'entrée + sous-commandes réelles `fk`/`validate-limits`
...
```bash
# Linux / macOS
./build.sh   # incrément de version compteur kilométrique, `cargo test` (24 tests), puis `cargo build --release`
...
`fk` se termine avec `0` en cas de succès, `2` si `--urdf`/`--joints` est invalide. `validate-limits` se termine avec `0` (aucune violation), `1` (violations trouvées), ou `2` (entrée invalide).
