...
```mermaid
flowchart LR
    URDF["URDF visuel - réel v0 (parcours d'arbre parent/child, chaînes sérielles uniquement)"] --> MESH["Simplification du maillage de collision - prévu"]
...
* **Pourquoi v0 refuse un vrai URDF ramifié plutôt que de l'aplatir.** Trouvé lors d'un audit d'amélioration logicielle à l'échelle de l'écosystème : v0 analysait auparavant les éléments `<joint>` dans l'ordre brut du document et traitait cet ordre comme la chaîne, ce qui est incorrect pour tout URDF conforme aux spécifications déclarant ses articulations hors de l'ordre de l'arbre. Corrigé : les vrais noms de liaison `parent`/`child` de chaque articulation sont désormais parcourus depuis la vraie liaison racine du document dans un ordre racine-vers-extrémité authentique. `Chain` reste un `Vec<Joint>` plat, donc une vraie ramification (une liaison avec plus d'une articulation enfant) n'a aucun ordre unique pour l'aplatir - elle est refusée avec une vraie erreur plutôt que de garder silencieusement une branche et d'abandonner l'autre. Honnête pour le propre catalogue de `HYDRA-UMC-EDITOR-URDF`, entièrement composé de bras sériels uniques aujourd'hui (voir `urdf.rs`).
...
│   ├── urdf.rs           # Lecteur URDF réel et partiel (parcours d'arbre parent/child, chaînes sérielles uniquement)
...
```bash
# Linux / macOS
./build.sh   # incrément de version compteur kilométrique, `cargo test` (49 tests), puis `cargo build --release`
